<template>
  <div class="container max-w-3xl mx-auto py-8">
    <h2 class="text-2xl font-bold text-gray-900 sm:text-3xl mb-4 text-center">
      {{ type == 'edition' ? 'Édition d\'un rôle' : 'Création d\'un rôle' }}
    </h2>
    ...
    <div class="w-full shadow p-5 sm:rounded-lg bg-white">
      <div class="relative">
        ...
        >
          <p class="mb-4">
            <span class="underline">Attention</span>&nbsp;: il est à noter que les rôles ajoutés,
            en plus des rôles <span class="font-bold">Enseignant</span> et <span class="font-bold">Administrateur</span>, n'auront pas de droits spécifiques à travers l'application.
            Leur utilisation sert uniquement à regrouper un ensemble d'utilisateurs sous un label.
          </p>
...
export default {
    ...
    data () {
        return {
            type: 'creation',
            id_role: null, // L'id de l'utilisateur si on est en édition
    ...
    created () {
        ...
        if (this.type === 'edition') {
            promises.push(APIService.get('admin/role/' + this.id_role).then(result => {
                this.form.name = result.data.display_role;
            }));
        }
    ...
    methods: {
        ...
        onSubmit() {
            ...
            if (this.type === 'creation') {
                APIService.post('admin/role/create', payload).then(result => {
                    if (result.message === "INSERT_OK"){
                        this.$router.push({ name: 'AdminView' });
                        this.initForm();
                        this.$toast.success("Le rôle a bien été créé.");
                        this.$store.dispatch('user/setLoading', false);
                    } else {
                        this.$toast.error("Une erreur est survenue lors de la création du rôle.");
                        this.$store.dispatch('user/setLoading', false);
                    }
                });
            }
            ...
            if (this.type === 'edition') {
                APIService.post('admin/role/' + this.id_role + '/edit', payload).then(result => {
                    if (result.message === "EDIT_OK"){
                        this.$router.push({ name: 'AdminView' });
                        this.initForm();
                        this.$toast.success("Le rôle a bien été modifié.");
                        this.$store.dispatch('user/setLoading', false);
                    } else {
                        this.$toast.error("Une erreur est survenue lors de la modification du rôle.");
                        this.$store.dispatch('user/setLoading', false);
                    }
                });
            }
