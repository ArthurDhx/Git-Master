<template>
  <div class="container max-w-3xl mx-auto py-8">
    <h2 class="text-2xl font-bold text-gray-900 sm:text-3xl mb-4 text-center">
      {{ type == 'edition' ? 'Édition d\'une année' : 'Création d\'une année' }}
    </h2>
...
export default {
    ...
    data () {
        return {
            type: 'creation',
            id_year: null, // L'id de l'utilisateur si on est en édition
    ...
    created () {
        ...
        if (this.type === 'edition') {
            promises.push(APIService.get('admin/year/' + this.id_year).then(result => {
                this.form.name = result.data.label_year;
            }));
        }
    ...
    methods: {
        ...
        onSubmit() {
            ...
            if (this.type === 'creation') {
                APIService.post('admin/year/create', payload).then(result => {
                    if (result.message === "INSERT_OK"){
                        this.$router.push({ name: 'AdminView' });
                        this.initForm();
                        this.$toast.success("L'année  a bien été créé.");
                        this.$store.dispatch('user/setLoading', false);
                    } else {
                        this.$toast.error("Une erreur est survenue lors de la création d'une année.");
                        this.$store.dispatch('user/setLoading', false);
                    }
                });
            }
            ...
            if (this.type === 'edition') {
                APIService.post('admin/year/' + this.id_year + '/edit', payload).then(result => {
                    if (result.message === "EDIT_OK"){
                        this.$router.push({ name: 'AdminView' });
                        this.initForm();
                        this.$toast.success("L'année a bien été modifié.");
                        this.$store.dispatch('user/setLoading', false);
                    } else {
                        this.$toast.error("Une erreur est survenue lors de la modification d'une année.");
                        this.$store.dispatch('user/setLoading', false);
                    }
                });
            }
